...
## Settings `ToggleRow`: description length is no longer layout-constrained

`ToggleRow`/`ToggleSwitch` (`Settings.tsx`) used to vertically center the
switch against the *whole* label+description text block via plain flexbox
`align-items: center` — that only looked consistent row-to-row as long as
every row's description wrapped to the same number of lines. Two toggles
were added with long descriptions that wrapped to 2 lines (the Last.fm
section's "Show Recently Played", and earlier the Playback tab's
"Scrobble") while every other row in the same section stayed at 1 line —
centering against a taller 2-line block put the switch at a visibly
...
**Fixed at the component level.** `ToggleRow` now nests the switch in its
own row with *only* the label (`flex items-center justify-between`), and
renders the description as a separate line underneath, outside that row.
The switch centers against the single-line label only, so it stays pinned
to the same vertical spot no matter how long the description is or how
many lines it wraps to. Description copy no longer needs to be trimmed to
fit one line at the Settings panel's `maxWidth: 480` column width — write
whatever length actually explains what the toggle does.
